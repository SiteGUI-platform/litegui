			...
			sgView = new function() {
				...
				var converters = {
					translate: {
						...
						to: function(value) {
							return $('<p/>').text( Sitegui.trans(value) ).html() //return escaped HTML
						}
					},
					currency: {
						from: function (value) { return value },
			    ...
			    links: function(column, row){
			      ...
			      var thisLinks = '';	
						if (ajaxResponse.links.edit){
							let editLink = (row.type == 'Collection' && ajaxResponse.links.collection)? ajaxResponse.links.collection : ajaxResponse.links.edit
							thisLinks += $('<a class="btn-group"><button type="button" class="btn btn-sm border-0 text-primary btn-outline-light rounded-circle"><i class="bi {if $html.file_manager}bi-pencil{else}bi-card-text{/if}"></i></button></a>')
								.attr('href', editLink +'/'+ row.id +'{if $system.sgframe}?sgframe=1{/if}')
								.wrap('<p/>')
								.parent()
								.html()
						}
						if (ajaxResponse.links.copy){
							thisLinks += $('<a class="btn-group"><button type="button" class="btn btn-sm border-0 text-primary btn-outline-light rounded-circle"><i class="bi bi-files"></i></button></a>')
								.attr('href', ajaxResponse.links.copy +'/'+ row.id +'{if $system.sgframe}?sgframe=1{/if}')
								.wrap('<p/>')
								.parent()
								.html()
						}
						...
						if (ajaxResponse.links.manage){
							let data_url = ajaxResponse.links.manage +'/{$page.subtype|default:$page.type|lower}/{$page.id}/'+ ajaxResponse.html.current_app.toLowerCase() +'/'+ row.id
							data_url += ajaxResponse.links.publish? '?published='+ (row.published ?? '') : ''
							{if $system.sgframe}
								data_url += ajaxResponse.links.publish? '&sgframe=1' : '?sgframe=1'
							{/if}//

							thisLinks += $('<button type="button" class="btn btn-sm border-0 text-primary btn-outline-light rounded-circle js-propagate"  data-confirm="delete" data-cors="1"><i class="bi bi-gear"></i></button>')
								.attr('data-url', data_url)
								.wrap('<p/>')
								.parent()
								.html()							
						}
						if (ajaxResponse.links.publish){
							thisLinks += $('<button type="button" class="btn btn-sm border-0 text-secondary btn-outline-light rounded-circle"><i class="bi bi-globe2"></i></button>')
								.attr('data-publish', row.id)
								.addClass(row.published > 0? 'text-info' : '')
								.wrap('<p/>')
								.parent()
								.html()
						}
			    	if (ajaxResponse.html && ajaxResponse.html.app_tick && ajaxResponse.html.app_tick.params && ajaxResponse.links.update){
							thisLinks += $('<button type="button" class="btn btn-sm border-0 text-primary btn-outline-light rounded-circle js-propagate" data-checked="'+ row.id +'"><input class="form-check-input border-warning bg-warning align-baseline" type="checkbox"></button>')
									//.attr('href', ajaxResponse.links.edit +'/'+ row.id +'{if $system.sgframe}?sgframe=1{/if}')
								.attr('title', ajaxResponse.html.app_tick.label)
								.wrap('<p/>')
								.parent()
								.html()
						}	
			      return '<div class="btn-toolbar float-end flex-nowrap position-relative">'+ thisLinks +'</div>';
					...
					html: function(column, row){ 
					  ...
					  if ( column.id == 'creator' || column.id == 'creator_name' ){ //creator name convert to link
					  	let abbr
							if (isNaN(row.creator_name) && !row.creator_avatar && Object.keys(row).length == 4){ //grid only
								abbr = row.creator_name.split(' ').map((n)=>n[0]).join('').toUpperCase();
								abbr = $('<div class="sg-datatable-thumb">').text(abbr)
							} else {
								abbr = ''
							}
							return $('<a href="#" data-bs-toggle="modal" data-bs-target="#dynamicModal" class="text-decoration-none opacity-75"></a>')
								.attr('data-url', (ajaxResponse.links.datatable.creator || links.creator_name) +'/'+ row.creator +'?sgframe=1')
								.attr('data-title', row.creator_name)
								.attr('title', row.creator_name)
								.attr('href', '#'+ row.creator_name)
								.text(Object.keys(row).length != 4? row.creator_name : '') //not in grid
								.prepend(row.creator_avatar? 
									$('<img class="rounded-circle me-1 sg-datatable-thumb" loading="lazy">')
									.attr('src', row.creator_avatar) : abbr 
								)
								.wrap('<p/>')
								.parent()
								.html()
						} else if ( row[column.id] ){
							if (typeof row[column.id] === 'object') { //load via ajax
								obj = Object.keys(row[column.id]).length? row[column.id] : {};
							...
							for (var item in obj) {
								if (obj.hasOwnProperty(item)){
									...
									value += ', ' + $('<a href="#" data-bs-toggle="modal" data-bs-target="#dynamicModal" class="text-decoration-none opacity-75"></a>')
										.attr('data-url', href + item +'?sgframe=1')
										.attr('data-title', obj[item])
										.attr('title', obj[item])
										.attr('href', '#'+ obj[item])
										.prepend(
			    ...
			    responseHandler: function (response) {
			    	...
			    	if ( response.rows.length ){
				    	if (response.html.display == 'flat') {			    	
			    			$(that.container +'-footer').attr('id', that.container +'-footer-show')	//change id to prevent hiding by loaded event
									/* not support searching content atm .find('.infoBar')
									.before( 
										$('<div class="col" />').append( 
											$(that.container +'-header').find('.searchBar') 
										) 
									)*/	
			    			$(that.container +'-wrapper').removeClass('card d-none')
				    		...
				    		let t, template = '<div class="col-12 mt-3 mx-auto">\
				    			<div class="card">\
				    				<div class="row g-0">\
				    		...
				    		response.rows.forEach( $row => {
			    				...
			    				if ($row.creator){
			    					t.find('.sg-creator').text( $row.creator_name ).attr('href', response.links.datatable.creator +'/'+ $row.creator)
										{if $forapp}
			    				...
			    				$(formatters.links(null, $row))
			    					.addClass('justify-content-end col')
			    					.insertAfter( t.find('.sg-date').parent() )
			    				...
			    				if (Object.keys($subapp).length) {
			    					...
			    					if (ajaxResponse.links && ajaxResponse.links.edit){
			    						$('<a class="py-0" href="#" data-bs-toggle="modal" data-bs-target="#dynamicModal"><i class="bi bi-patch-plus"></a>')
			    							.attr('data-url', ajaxResponse.links.edit + '/'+ $row.id + '?sgframe=1&add=1')
							...
							$('body')
								.off('click.sg.datatable', '[data-checked]') //remove previous event listener using namespace 
								.on('click.sg.datatable', '[data-checked]', function(ev) {
									if ( response.html.app_tick && ajaxResponse.html.app_tick.params && response.links.update && $(ev.currentTarget).find('input').is(':checked') ){
										let href = response.links.update;
						        let post = 'page%5Bid%5D='+ $(this).attr('data-checked') 
						        	+'&page%5Bsubtype%5D='+ response.html.current_app
						        	+'&'+ response.html.app_tick.params 
			    	...
			    	if (response.total > 0 || response.html.user_engaged) {
			    		...
			    		if (response.total > 0 && !btn.is('.sg-no-badge')){
			    			btn.append( $('<span class="position-absolute top-0 start-75 translate-middle badge bg-light rounded-pill text-secondary"/>').text(response.total) )
			    		}	
			    	}
			    	...
			    	if ( response.html.table_header ){ 
			    		...
			    		if ( this.app != response.html.current_app ){
				            ...
				            headerAlign: 'left',
				            cssClass: '',
				            headerCssClass: '',//sticky-top bg-white',
				            formatter:  null,
				            order: null,
				    		...
				    		if (response.html.kanban) {
				    			$view.children('.dropdown-menu').prepend('<li><hr class="dropdown-divider"></li><li><button type="button" class="btn border-0 ps-3 disabled">{"Kanban"|trans}</button> <button type="button" class="btn border-0 float-end pe-4 disabled"><i class="bi bi-kanban"></i></button></li><li><a class="dropdown-item" href="#" data-column="month">{"Month"|trans}</a></li><li><a class="dropdown-item" href="#" data-column="year">{"Year"|trans}</a></li>')
				    		}	
				    		$view.children('.dropdown-menu')
				    			.prepend('<li><hr class="dropdown-divider"></li><li><div class="dropdown ps-1" data-view="grid"><button type="button" class="btn border-0">{"Grid"|trans}</button><button type="button" class="btn border-0 float-end pe-4"><i class="bi bi-grid"></i></button></div></li>')
				    			.prepend(
				    				$('<li />').append(
					    				$('<div class="dropdown ps-1"/>')
						    			.append('<button type="button" class="btn border-0" data-view="table">{"Table"|trans}</button>')
						    			.append(
						    				$('<button type="button" class="btn border-0 dropdown-toggle dropdown-toggle-split float-end" data-bs-toggle="dropdown" aria-expanded="false" data-bs-reference="parent"><i class="bi bi-card-checklist"></i> <span class="visually-hidden">{"Toggle Dropdown"|trans}</span></button>')
			    ...
			    templates: { 
			    	header: '<div id="{literal}{{ctx.id}}" class="{{css.header}}{/literal} fade show">\
				    	<div class="row">\
				    		...
				    		{literal}\
				    		<div class="col col-sm-4 d-flex d-md-block justify-content-end"><p class="{{css.search}}"></p></div>\
				    		<div class="col-12 col-sm-auto px-0 force-new-line-sm"></div>\
				    		<div class="col-6 col-sm-2 pt-1"><p class="{{css.pagination}}"></p></div>\
				    		<div class="col-6 col-sm-2 text-end pt-1"><p class="{{css.actions}}"></p></div>\
				    	</div>\
			    	</div>',

			    	search: '<div class="form-group searchBar"><div class="input-group"><span class="input-group-text bg-transparent border rounded-start {{css.icon}} {{css.iconSearch}}"></span> <input type="text" class="{{css.searchField}} border rounded-end shadow-none" placeholder="{{lbl.search}}" /></div></div>',
            ...
            all: "{'All'|trans}"
				...
				this.initBootgrid = function (container, apiUrl) {
					...
					$(container).bootgrid(bootgridOptions).on("loaded.rs.jquery.bootgrid", function(e){						
						$(container +'-wrapper .js-propagate').click(function(ev) {
				...
				var kanbanGroupBy = function (ajaxResponse, $col) {
					...
					for (let $i=0; $i < $arr.length; $i++){
						...
						let $item = {
							...
							url: ajaxResponse.links.edit +'/'+ $arr[ $i ]['id'] +'?sgframe=1',
							'bs-toggle': "modal",
							'bs-target': "#dynamicModal",
						}
						if ( ajaxResponse.html.app_tick && ajaxResponse.html.app_tick.params ){
							$item.tick = (ajaxResponse.html.app_tick.label || 'Tick') 
						}	
						if ( $arr[ $i ]['creator'] ){
								...
								if ( ! boards[ $col ][ $label2 ] ){
									...
									if ( ! boards[ $col ][ $label2 ] ){
										...
										if ($col == 'month') {
											...
											boards[ $col ][ $label2 ]['name'] = Sitegui.trans($label2)
										} else {
			    ...
			    dropEl           : function (el, target, source, sibling) {
			        ...
			        $(target).parent().removeClass('fold was-fold')
			        $(el).prepend('<div class="sg-status fade show float-end" role="status"><div class="spinner-grow spinner-grow-sm text-info" role="status"><span class="visually-hidden">{"Saving"|trans}...</span></div></div>')
		            ...
		            if (response.status.result == 'success'){
		            	...
		            	$(target).parent().find('.kanban-board-count').text('('+ $(target).find('.kanban-item').length +')')
		            	$(source).parent().find('.kanban-board-count').text('('+ $(source).find('.kanban-item').length +')')
		            	setTimeout(function(){
			    ...
			    buttonClick      : function(el, boardId) {
			    	if ( $(el).closest('.kanban-board').hasClass('fold') ) {
			    		...
			    		$(el)//.addClass('float-start')
			    			...
			    			.find('.kanban-board-count').text(
			    				'('+ $(el).closest('.kanban-board').find('.kanban-drag .kanban-item').length +')'
			    			)	
			    	}

			    },                     // callback when the board's button is clicked
			    propagationHandlers: ["click"],                                         // the specified callback does not cancel the browser event. possible values: "click", "context"
				}
				//kanban view
			    ...
			    kanbanOptions.ajaxResponse = ajaxResponse
					...
					if ( $.inArray($col, ["month", "year", "creator"]) > -1 || !ajaxResponse.links.update){
						kanbanOptions.dragItems = false
					}	else {
						kanbanOptions.dragItems = {if $forapp}false{else}true{/if}{*/*}
					}
					...
					for (let $i=0; $i < $arr.length; $i++){
						if ( !type || !$arr[ $i ]['subtype'] || $arr[ $i ]['subtype'] === type){
							...
							if (title){
								...
								if (ajaxResponse.links.edit){
									item.find('.sg-title').wrap('<a class="text-decoration-none">')
										.parent()
										.attr('href', ajaxResponse.links.edit +'/'+ $arr[ $i ]['id'] +'{if $system.sgframe}?sgframe=1{/if}')
								}
							} else {
				    	...
				    	sgView.showHeaderFields($arr[ $i ], ajaxResponse, item.find('.card-body'), 1)	
							...
							if ( ajaxResponse.subapp && ajaxResponse.subapp.count && ajaxResponse.subapp.show ){
								Object.keys(ajaxResponse.subapp.show).forEach( $subapp => {
									...
									if (ajaxResponse.subapp.count[ $key ] ){
										...
										$count += Sitegui.trans( (ajaxResponse.subapp.count[ $key ] > 1)? ajaxResponse.subapp.show[ $subapp ]['plural'] : ajaxResponse.subapp.show[ $subapp ]['single'] )
										$count += ' '
									}				
								})
								if ($count) {
							...
							if ( $arr[ $i ]['published'] || $arr[ $i ]['expire'] || $arr[ $i ]['registered'] || $arr[ $i ]['created'] || $arr[ $i ]['updated'] || $arr[ $i ]['due'] || $arr[ $i ]['due_date'] || $arr[ $i ][ ajaxResponse.html.current_app.toLowerCase() +'_due'] || $arr[ $i ][ ajaxResponse.html.current_app.toLowerCase() +'_due_date'] ){
...
{if !$forapp}
	{block 'content_spotlight'}{$content_spotlight nofilter}{/block}
	{block 'content_left'}
	<div class="col-auto px-0 content_left">{$content_left nofilter}</div>
	{/block}
{/if}	
<div class="col">
	<div class="row">		
		{if !$forapp}{block 'content_top'}{$content_top nofilter}{/block}{/if}
		<div class="col-12 mx-auto" id="{$table}-content-wrapper"></div>

		<div class="col-12 {if $system.sgframe}px-0{elseif $table eq datatable}col-md-10 pt-sm-2{/if} {if $content_header}col-md-12{/if} mx-auto">
			<div class="card datatable-wrapper overflow-x {if $forapp AND $app.sub.$forapp.display == flat}d-none{/if}" id="{$table}-wrapper">
			...
			{block 'content_main'}{$content_main nofilter}{/block}
			<div id="{$table}-grid" class="row row-cols-2 row-cols-sm-3 row-cols-md-4 g-3 mt-2 mb-4 {if $system.sgframe}mx-1{/if} d-none">
		    <div class="item col d-none" data-filter="subtype">
		      <div class="thumbnail card h-100 {if $forapp OR $system.sgframe}bg-light bg-opacity-10{else}border-0{/if}">
		        ...
		        <div class="caption card-footer pe-2 border-top-0 {if $forapp OR $system.sgframe}bg-transparent{else}bg-white{/if}">
		          <div class="row">
		...
		<div class="kanban-wrapper col-12 mx-auto py-4" id="{$table}-kanban-wrapper" style="overflow-x:auto"></div>
		{if !$forapp}{block 'content_bottom'}{$content_bottom nofilter}{/block}{/if}
	</div>	
</div>
{if !$forapp}
	{block 'content_right'}
	<div class="col-auto px-0 content-right">{$content_right nofilter}</div>
	{/block}
{/if}	

{if !$system.sgframe}
	{block name="data_footer"}
	<div class="col-12 mx-auto pb-20 default-padding"></div>	
	{/block}
{/if}	
<script type="text/javascript">
  document.addEventListener("DOMContentLoaded", function(e){				
		...
		{if ($api.rowCount AND !$api.rows) OR $html.ajax}
			...
			const statusParam = (new URLSearchParams(window.location.search) ).get('status'); //load specific status
			apiUrl += statusParam? '&status='+ statusParam : ''
		{/if} {*/*}
		//{$datatable_onclick}	
		{if !$data_onclick}	
	    ...
	    {if $api.app.sub.$forapp.entry == multiple OR $api.app.sub.$forapp.entry == single OR 
	       ...
	       ($api.app.sub.$forapp.entry == creator_readonly AND $api.page.creator AND $api.page.creator != $user.id) OR  
	       ($api.app.sub.$forapp.entry == other_readonly AND $api.page.creator == $user.id) 
				...
				$('#{$table}-header .sg-app-header').append( 
					$('<button class="btn btn-outline-primary" type="button" data-bs-toggle="collapse" aria-expanded="false"></button>')
						...
						.text('{"New :item"|trans:["item" => $api.app.sub.$forapp.alias|default:($forapp|replace:"_":" ")] }') 
					)
			{/if}		
		{/if}

		if (window.innerWidth < 600 && document.querySelector('.modal-fullscreen') ){
			document.querySelector('.modal-fullscreen').style.width = null;
		}
		$('body').on('click', '.js-sg-expand', function(){
			$(this).closest('.datatable-wrapper').parent().toggleClass('col-md-12')
			$('body').find('.kanban-board').removeClass('fold')
		})
	});		
</script>
{if !$forapp}
<script defer src="{$system.cdn}/{$template}/assets/js/postmessage.js?v=6" id="sg-post-message" data-origin="{$system.edit_url}"></script>{/if}	
{/if}