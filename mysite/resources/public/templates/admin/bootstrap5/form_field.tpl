      ...
      {elseif $field.type == 'radio hover'}
        <div class="sg-radio-hover" role="button" {if $field.visibility == readonly AND $field.value}disabled{/if}>
          ...
          {foreach $field.options AS $label}
            <input type="radio" name="{$fieldPrefix}[{$key}]" id="rhover{$form_script_loaded}-{$keyid}{$label@index}" value="{$label}" {if $label eq $field.value OR $label@first}checked{/if} {if $field.visibility == readonly AND $field.value}disabled{/if}><label for="rhover{$form_script_loaded}-{$keyid}{$label@index}">{$label}</label>
          {/foreach}
        </div>
        <style type="text/css">
