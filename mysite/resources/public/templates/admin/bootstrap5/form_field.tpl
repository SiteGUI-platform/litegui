{function buildForm}
  ...
  {if $field.type == 'header'}
    ...
    {if $field.description}<small class="form-text text-secondary col-12 pb-3">{$field.description}</small>{/if}    
  {else} 
    <label class="col-sm-{$c1|default:3} col-form-label text-sm-end">
      {$field.label|trans} 
      {if $field.type == 'select' AND $field.slug}
        {foreach $field.value AS $id => $value}
          {if $id} 
          [<a href="#{$value}" data-url="{$field.slug}/{$id}?sgframe=1" title="{$value}" data-title="{$value}" data-bs-toggle="modal" data-bs-target="#dynamicModal" class="text-decoration-none" target="_blank">{if $field.images.$id}<img class="rounded-circle sg-datatable-thumb float-none" loading="lazy" src="{$field.images.$id}">{else}{$value@index+1}{/if}</a>]
          {/if}
        {/foreach}  
      {/if}
      {if $field.is == required}*{/if}
    </label>
    <div class="col-sm-{$c2|default:7}">
      {if $field.type == 'lookup'}
        <div {if $field.visibility != readonly || $field.value !== (array) $field.value}class="input-group dropup" {if $field.slug}data-slug="{$field.slug}"{/if}{/if}>
        {if $field.visibility != readonly || $field.value !== (array) $field.value}
          <button class="input-group-text bg-body rounded-start" type="button"><i class="bi bi-search"></i></button>
          <input class="form-control lookup-field bg-body dropdown-toggle rounded-end" type='text' placeholder="{'Name'|trans}" data-lookup="{$field.lookup_key|default:$key}" data-index="{$form_script_loaded}" {if $field.scope}data-scope="{$field.scope}"{/if} data-name="{$fieldPrefix}[{$key}]{if $field.is == multiple}[ ]" data-multiple="1{/if}" data-bs-auto-close="outside" data-bs-toggle="dropdown" aria-expanded="false">
          <div class="dropdown-menu px-2" style="max-height:50vh; overflow: scroll;">{if $field.value}
            ...
            {if $field.is != multiple}
              <div class="form-check"><input class="form-check-input lookup-item" type="radio" id="{$keyid}-lookup{$form_script_loaded}-none" name="{$fieldPrefix}[{$key}]" value=""><label class="form-check-label" for="{$keyid}-lookup{$form_script_loaded}-none">{"None"|trans}</label></div> 
            {/if}
          {/if}</div>
        {/if}
        {if $field.value === (array) $field.value AND ($field.value|count > 1)}
          </div>
        {/if}  
        {foreach $field.value as $id => $value}
          {if $field.is == multiple || $value@last}
          <label class="sg-lookup-result bg-body rounded
            {if $field.value AND ($field.value|count > 1)}
              border me-1 mt-2 p-1 ps-3
            {else}
              input-group-text
            {/if}
          ">
            {if $field.images.$id}
              <img class="rounded-circle me-1 sg-datatable-thumb" loading="lazy" src="{$field.images.$id}">{/if}
            {if $field.slug}
              <a href="#" data-url="{$field.slug}/{$id}?sgframe=1" data-title="{$value}" data-bs-toggle="modal" data-bs-target="#dynamicModal" class="text-decoration-none" target="_blank">{$value}</a>
            {else}
              {$value}
            {/if}
            {if $field.visibility != readonly}
              <label for="{$keyid}-lookup{$form_script_loaded}-{if $field.is == multiple}{$id}{else}none{/if}"><i class="text-primary bi bi-x ps-1 pt-1" role="button"></i></label>
            {/if}  
          </label>
          {/if}
        {/foreach}
        {if !$field.value OR $field.value !== (array) $field.value OR (($field.value|count) eq 1)}
          </div>      
        {/if}
      {elseif $field.type == 'file'} 
        {foreach $field.value AS $file}
          <div class="input-group pb-2">
            <span class="input-group-text bg-body rounded-start" role="button" data-bs-toggle="modal" data-bs-target="#dynamicModal" data-url="{if $file|truncate:4:'' ne 'http'}{$links.file_view}/{/if}{$file}" data-title="{'Preview'|trans}"><i class="bi bi-eye"></i></span>
            <input class="form-control file-upload-indicator bg-body border-end-0" type="text" id="fid{$form_script_loaded}-{$keyid}-{$file@index}"  data-upload-name="{$fieldPrefix}[{$key}][ ]" name="{$fieldPrefix}[{$key}][ ]" value="{$file}" readonly>
            {if $html.file_manager AND ($field.visibility != readonly OR !$field.value)} 
              <span class="input-group-text bg-body get-file-callback pe-auto" role="button" data-container="#fid{$form_script_loaded}-{$keyid}-{$file@index}" data-folder="{$html.upload_dir}" data-bs-toggle="modal" data-bs-target="#dynamicModal" data-title="{'File Manager'|trans}"><i class="bi bi-upload"></i></span> 
            {/if} 
            {if $field.visibility != readonly}
              <button class="input-group-text bg-body self-remove" type="button" ><i class="bi bi-x-lg"></i></button>
            {/if}  
          </div>    
        {/foreach} 
        ...
        <div class="input-group pb-2"> {* PHP's assign file array unintuitive, do not use prefix *}
          ...
          {if $html.file_manager}
            type="text" name="{$fieldPrefix}[{$key}][ ]" data-container="#fid{$form_script_loaded}-{$keyid}" data-folder="{$html.upload_dir}" data-bs-toggle="modal" data-bs-target="#dynamicModal" data-title="{'File Manager'|trans}"
          {else}
            type="file" name="{$fieldPrefix}[{$key}][ ]" accept="image/*"
          {/if} {if $field.is == required && $field.visibility != hidden}required{/if}>
          <label class="input-group-text bg-body" for="fid{$form_script_loaded}-{$keyid}" role="button"><i class="bi bi-upload"></i></label>
          {if $field.is == multiple}<span class="input-group-text bg-body add-another-input" role="button" ><i class="bi bi-plus-lg"></i></span>{/if}
        </div>
        {/if}
      {elseif $field.type == 'image'}
        <div class="carousel-multi pb-1 w-100 {if $field.is == multiple}multiple-values{/if}" data-length="3" data-slide-to="0" data-noloop="1" data-dynamic="1">
          <div id="target{$form_script_loaded}-{$keyid}" class="carousel-inner {if $field.visibility != readonly}item-removable{/if} row gx-0">
            {foreach $field.value as $image}
            <div class="col position-relative">
              {if $image}<img src="{if $image|truncate:4:'' ne 'http'}{$links.file_view}/{/if}{$image}" class="img-thumbnail p-0 d-block mx-auto">{/if}<input type="hidden" name="{$fieldPrefix}[{$key}][ ]" value="{$image}">
            </div>
            {/foreach}                
          </div>    
        ...
        <div class="input-group pb-2 {if $field.is != multiple && $field.value}d-none{/if}">
          ...
          {if $html.file_manager}
            type="text" name="{$fieldPrefix}[{$key}][ ]" data-folder="{$html.upload_dir}" data-bs-toggle="modal" data-bs-target="#dynamicModal" data-title="{'File Manager'|trans}"
          {else}
            type="file" name="{$fieldPrefix}[{$key}][ ]" accept="image/*"
      ...
      {elseif $field.type == 'checkbox'}
        ...
        <label class="col-form-label" for="cid{$form_script_loaded}-{$keyid}-1">{$field.description}</label>
      ...
      {elseif $field.type == 'select'}
        ...
        {/if}  
        <select class="form-control {if $field.visibility != readonly OR !$field.value}selectpicker show-tick d-block{/if}" {if $field.options && $field.options|count > 10}data-live-search="true"{/if} data-style="form-control border" data-none-selected-text="&nbsp;" name="{$fieldPrefix}[{$key}]{if $field.is == multiple}[ ]" multiple{else}"{/if} {if $field.is == required && $field.visibility != hidden}required{/if} {if $field.visibility == readonly && $field.value}disabled{/if} {if $field.listen && $html.file_manager}data-lookup="{$field.lookup}" data-lookup-input="\[{$field.listen}\]" {if $field.scope}data-scope="{$field.scope}"{/if}{/if}>
          {if !$field.is OR $field.is eq optional}<option/>{/if}
          {if $field.value == (array) $field.value}
            {foreach $field.value AS $option => $label} {* this will help display selected values no longer in the options *}
      ...
      {elseif $field.type == 'radio'}
        {foreach $field.options AS $option => $label}
          ...
          <div class="form-check col-form-label">
            ...
            <label class="form-check-label" for="field{$form_script_loaded}-{$option}">{$label}</label>
          </div>
          {/if}
        {/foreach}
      {elseif $field.type == 'radio hover'}
        <div class="sg-radio-hover" role="button" {if $field.visibility == readonly AND $field.value}disabled{/if}>
          ...
          {foreach $field.options AS $label}
            <input type="radio" name="{$fieldPrefix}[{$key}]" id="rhover{$form_script_loaded}-{$keyid}{$label@index}" value="{$label}" {if $label eq $field.value OR $label@first}checked{/if} {if $field.visibility == readonly AND $field.value}disabled{/if}><label for="rhover{$form_script_loaded}-{$keyid}{$label@index}" role="button">{$label}</label>
          {/foreach}
        </div>
        <style type="text/css">
      ...
      {elseif $field.type == 'rating' OR $field.type == 'percentage'}
        <input class="sg-rating mt-1" type="range" role="button" 
      ...
      {elseif $field.type == 'date' OR $field.type == 'time'}
        {if $field.visibility == readonly AND $field.value}
          <div class="form-control js-sg-{$field.type}">{$field.value}</div>
        {else}  
          <div class="input-group">
          ...
          {/if}
        {/if}  
      {elseif $field.type == 'oauth' AND $field.options.0}
        <div class="">
          {if $field.authorized}
            {'Authorized'|trans} <i class="bi bi-check-lg me-3"></i>
          {/if}
          <div class="btn btn-outline-primary" {if $field.visibility != readonly || !$field.value}onclick="window.open('{$field.options.0}')"{/if}>
            <i class="bi bi-person-vcard"></i> {'Authorize'|trans}
          </div>
        </div>        
      ...
      {else} {* $field.type == 'text' || $field.type == 'password' || $field.type == 'url' || $field.type == 'email' || $field.type == 'tel' || $field.type == 'color' || $field.type == 'currency' *}
        <div class="input-group position-relative">
          {if $field.type == 'currency' AND $site.currency.prefix}<span class="form-control position-absolute ps-0">{$site.currency.prefix}</span>{/if}
          ...
          {if $field.type == 'currency' AND $site.currency.suffix}<span class="form-control position-absolute end-0">{$site.currency.suffix}</span>{/if}
        </div>  
      {/if}
      {if $field.description AND $field.type != 'checkbox'}<div class="form-text text-secondary mt-2">{$field.description}</div>{/if}      
    </div>
  {/if}  
  </div> 
{/function}
...
{if ! $form_script_loaded}
  ...
  document.addEventListener("DOMContentLoaded", function(e){    
    $('.fieldset-container').on('click', '.new-fieldset', function(e) {
      ...
      newRow.find("[data-name]").each(function(){
      ...
      Sitegui.initTimePicker && Sitegui.initTimePicker(newRow) 
      Sitegui.initDatePicker && Sitegui.initDatePicker(newRow) 
    ...
    /* move to postmessage /select/upload image
    ...
    $('.carousel-multi').parent().on('updated', function (e) {
      input = $(this).find('input.get-image-callback');
...
{foreach $formFields as $name1 => $field1}
  {if $field1.type == 'fieldset' && $field1.fields}
    ...
    {foreach $field1.value as $index => $values}
      <div class="variant px-3 px-sm-0 alert alert-light alert-dismissible fade show" role="alert" data-index={$values@index}>
  ...
  {else}
    ...
    {if $field1.type == 'lookup' || $field1.listen}
      {include "lookup.tpl"}
    {/if}  
  {/if}  
{/foreach}