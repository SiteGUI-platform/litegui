  ...
  this.jKanban = function () {
        ...
        self.drake = self
          ...
          .on('drag', function (el, source) {
            ...
            if (boardJSON.dragTo !== undefined) {
              Object.keys(self.options.boards).map(function (index) {
                let board = self.options.boards[index]
                if (
    ...
    this.addBoards = function (boards, isInit) {
      ...
      for (var boardkey in boards) {
        ...
        headerBoard.classList.add('kanban-board-header', 'rounded-top-1', 'status__'+ (isNaN(board.id)? board.id.replaceAll(' ', '_').toLowerCase() : board.id) )
        allClasses.map(function (value) {
          // Remove empty spaces
          value = value.replace(/^[ ]+/g, '')
          headerBoard.classList.add(value)
        })
        headerBoard.innerHTML = '<div class="kanban-title-board">'+ __sanitizeHTML(board.name) + 
          ' <span class="kanban-board-count">('+ (board.item? Object.keys(board.item).length : 0) +')</span></div>'
        //content board
        ...
        for (var itemkey in board.item) {
          if (board.item.hasOwnProperty(itemkey)){ //work with array as well
            //create item
    ...
    function __findBoardJSON (id) {
      ...
      Object.keys(self.options.boards).map(function (index) {
        let board = self.options.boards[index]
        if (board.id == id) { //compare number with === is tricky
    ...
    function __appendCustomProperties (element, parentObject) {
      for (var propertyName in parentObject) {
        ...
        element.querySelector('.sg-title').setAttribute(
          'data-' + propertyName,
    ...
    function __buildItemCard(item) {
      var result = 'name' in item ? '<span class="sg-title d-block text-primary" role="button">'+ __sanitizeHTML(item.name) +'</span>' : '';
      if (item.tick) {
        result = '<button type="button" class="btn btn-sm js-propagate float-end p-0" data-checked="'+ 
          __sanitizeHTML(item.id) +'" title="'+ __sanitizeHTML(item.tick) +'"><input class="form-check-input" type="checkbox"></button>' + result
      }
      ...
      if (item.avatar){
        result += '<img class="rounded-circle me-1 sg-datatable-thumb" loading="lazy" src="'+ __sanitizeHTML(item.avatar) +'">'
      }
      result += __sanitizeHTML(item.creator??'') +'<i>&nbsp;</i></div>'
      result += '</div>'
      ...
      if (self.options.itemHandleOptions.enabled) {
          if ((self.options.itemHandleOptions.customHandler || undefined) === undefined) {
              ...
              result = '<div> ' + self.options.itemHandleOptions.customHandler.replace(/%([^%]+)%/g, function (match, key) 
                      { return item[key] !== undefined ? __sanitizeHTML(item[key]) : '' }) + ' </div>'
              return result
          }
      }
